...
star_packet* allocPacket(char* identifier, char* host, char* data) {
   // A packet.
   star_packet* packet = (star_packet*)malloc(sizeof(star_packet));
   ...
   packet->identifier = (char*)malloc(packet->identifier_len);
   packet->host = (char*)malloc(packet->host_len);
...
char* packetString(star_packet* packet) {
   // Create room for a string terminated by a cr and a \n
   int packetLen = HEADER_SIZE + packet->data_len + packet->identifier_len + 2;
   char* asciiPacket = (char*)malloc(packetLen);
   char cr = '\r';
   char lr = '\n';
   char asterisk = '*';
   int index = 0;
   ...
   memcpy(asciiPacket + packetLen - 1, &cr, 1);
   memcpy(asciiPacket + packetLen, &lr, 1);
...
star_packet* parsePacketString(char* packetString) {
   ...
   sender = (char*)malloc(senderLen + 1);
   ...
   identification = (char*)malloc(sizeof(char) * 5);
   identificationLen = 4;
